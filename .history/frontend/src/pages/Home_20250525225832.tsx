...
const Home: React.FC = () => {
  ...
  const [historySearchKeyword, setHistorySearchKeyword] = useState(''); // State for history report search keyword -> 历史报告搜索关键词的状态
    ...
    try {
      const response = await fetch('http://localhost:8000/save_report', {
        ...
        body: JSON.stringify({
          ...
          keyword: keyword // Add keyword to the request body -> 将关键词添加到请求体中
        }),
      });
      ...
      if (data.status === 'success') {
        ...
        setTimeout(() => {
          message.success({
            content: `报告已保存到指定目录：${data.filename}`, // Use filename from backend response -> 使用后端响应中的文件名
            duration: 3, // 显示3秒
            style: {
              position: 'absolute',
              top: '50%',
  ...
  }, [report, keyword]); // Add keyword as dependency -> 将 keyword 添加为依赖
    ...
    return (
      <div style={style} key={item.filename}>
        <AntList.Item // 使用 AntList.Item
          ...
          style={{ background: '#fff', borderRadius: '12px', marginBottom: '16px', padding: '20px', boxShadow: '0 2px 12px 0 rgba(0,0,0,0.05)' }} // 应用现有样式
        >
  ...
  }, [historyReports, viewHistoryReport, setEmailModalVisible]); // 添加依赖

  // Fetch history reports with optional keyword -> 获取带可选关键词的历史报告
      ...
      <div className={styles['search-bar']}>
        ...
        <AntButton
          ...
          onClick={() => {
            setShowHistory(true);
-            fetchHistoryReports();
+            fetchHistoryReports(historySearchKeyword); // Fetch with current keyword
          }}
      ...
      >
+        {/* Search input for history reports */}
+        <div style={{ marginBottom: '20px', display: 'flex', gap: '10px' }}>
+          <Input
+            placeholder="输入关键词搜索报告"
+            value={historySearchKeyword}
+            onChange={(e) => setHistorySearchKeyword(e.target.value)}
+            onPressEnter={() => fetchHistoryReports(historySearchKeyword)}
+            style={{ flexGrow: 1, borderRadius: '8px' }}
+          />
+          <AntButton type="primary" onClick={() => fetchHistoryReports(historySearchKeyword)} style={{ borderRadius: '8px' }}>
+            搜索
+          </AntButton>
+        </div>
+
