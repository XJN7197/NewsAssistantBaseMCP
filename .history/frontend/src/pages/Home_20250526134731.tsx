...
// 修改获取历史报告的函数
const fetchHistoryReports = useCallback(async (keyword?: string, pageNum: number = 1) => {
  setLoading(true);
  try {
    const url = `http://localhost:8000/reports?page=${pageNum}&page_size=${PAGE_SIZE}${keyword ? `&keyword=${encodeURIComponent(keyword)}` : ''}`;
    const response = await fetch(url);
    const data = await response.json();
    if (data.reports) {
      setHistoryReports(prev => pageNum === 1 ? data.reports : [...prev, ...data.reports]);
...
const handleScroll = useCallback(({ scrollOffset, scrollUpdateWasRequested }) => {
  if (!scrollUpdateWasRequested && !loading && hasMore && scrollOffset > 0) {
    const threshold = 100; // 距离底部多少像素时加载更多
    if (scrollOffset >= listRef.current.scrollHeight - threshold) {
      setPage(prev => prev + 1);
      fetchHistoryReports(historySearchKeyword, page + 1);
    }
  }
}, [loading, hasMore, page, historySearchKeyword]);
