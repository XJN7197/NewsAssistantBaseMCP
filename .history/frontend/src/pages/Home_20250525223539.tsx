    ...
    <div className={styles['home-container']}>
      ...
      >
        {loading ? (
          ...
          <VirtualList
            height={400} // 设置列表的高度
            itemCount={historyReports.length} // 列表项的数量
            itemSize={100} // 每个列表项的高度
            width={'100%'} // 列表的宽度
          >
            {({ index, style }) => (
              <div style={style}>
                <List.Item
                  actions={[
                    <AntButton type="link" onClick={() => viewHistoryReport(historyReports[index].filename)}>查看</AntButton>,
                    <AntButton
                      type="link"
                      onClick={() => {
                        viewHistoryReport(historyReports[index].filename).then(() => {
                ...
                >
                  <List.Item.Meta
                    title={<Title level={5} style={{ margin: 0, fontSize: '1.3rem', fontWeight: 600, color: '#1d1d1f' }}>{historyReports[index].filename}</Title>}
                    description={
                      <div>
                        <Text type="secondary" style={{ fontSize: '1rem', display: 'block', marginBottom: '8px' }}>
                          创建时间: {historyReports[index].created_at}
                        </Text>
