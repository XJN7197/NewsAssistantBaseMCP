...
async function fetchWithRetry(
  ...
  const doFetch = async (): Promise<Response> => {
    ...
    try {
      ...
      // 检查HTTP状态码，如果不是成功的范围（200-299），则抛出错误
      if (!response.ok) {
        // 对于非网络错误或超时错误，不进行重试
        if (response.status >= 400 && response.status < 500) {
           throw new Error(`Client error: HTTP status ${response.status}`);
        }
      ...
      // 判断是否需要重试：网络错误 (Failed to fetch) 或 超时错误 (AbortError)
      ...
      if (retryCount < maxRetries && (isNetworkError || isTimeoutError || isServerError)) {
        retryCount++;
        // 指数退避算法，增加重试间隔
        const delay = Math.pow(2, retryCount) * 1000 + Math.random() * 1000; // 增加随机抖动
        console.warn(`请求失败: ${error.message}. 正在进行第 ${retryCount} 次重试...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        return doFetch(); // 递归调用进行重试
      }

      // 达到最大重试次数或遇到不可重试的错误，抛出最终错误
      console.error(`请求最终失败: ${error.message}`);
      throw error; // 抛出原始错误以便调用者处理
    }
  };
...
const Home: React.FC = () => {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchWithRetry('http://localhost:8000/save_report', {
        method: 'POST',
        headers: {
  ...
  const fetchConfig = useCallback(async () => {
    ...
    try {
      // 使用 fetchWithRetry 替换 fetch
      const response = await fetchWithRetry('http://localhost:8000/config');
      const data = await response.json();
      if (data && data.config) {
  ...
  const viewHistoryReport = useCallback(async (filename: string) => {
    ...
    try {
      // 使用 fetchWithRetry 替换 fetch
      const response = await fetchWithRetry(`http://localhost:8000/reports/${filename}`);
      const data = await response.json();
      if (data.content) {
