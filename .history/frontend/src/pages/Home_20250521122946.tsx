...
import { List, Checkbox, Button as AntButton, Input, Spin, Modal, Typography } from 'antd'; // Ant Design 组件导入
...
const Home: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/analyze', { // 假设后端运行在 localhost:8000
        ...
        body: JSON.stringify({ news_list: newsToAnalyze }), // 将选中的新闻列表发送给后端
      });
  ...
  return (
    <div className={styles['home-container']}>
      ...
      >
        {report ? (
          <pre style={{ background: '#f5f5f7', border: '1px solid #e5e5e5', borderRadius: '14px', padding: '24px', fontSize: '1rem', color: '#333', overflowX: 'auto', lineHeight: 1.6 }}>
            {report}
          </pre>
        ) : (
          <Text>报告生成中或无内容...</Text>
