...
// 修改现有的 fetch 调用以使用新的 fetchWithRetry 函数
const handleSearch = useCallback(async () => {
  setLoading(true);
  setNewsList([]);
  setReport(null);
  ...
  try {
    ...
    if (import.meta.env.VITE_USE_MOCK_NEWS === 'true') {
      ...
      const response = await fetchWithRetry('http://localhost:8000/search', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    if (data && data.news_list) {
      const initialNewsList = data.news_list.map((news, index) => ({ ...news, id: index }));
      setNewsList(initialNewsList);
      const initialSelected = new Set(initialNewsList.map(news => news.id));
      setSelectedNews(initialSelected);
    } else {
      setNewsList([{ error: '未获取到新闻数据或数据格式不正确' }]);
    }
