  ...
  return (
    <div className={styles['home-container']}>
        ...
        {newsList.length > 0 && (
            <ul>
                {newsList.map((news, index) => (
                    <li key={index}>
                        {news.error ? (
                            <div style={{ color: 'red' }}>{news.error}</div>
                        ) : (
                            <>
                                <h4>{news.title}</h4>
                                <p>{news.desc}</p>
      ...
      <div className={styles['analyze-btn']}>
        <button onClick={handleAnalyze} disabled={loading || newsList.length === 0 || (newsList.length === 1 && newsList[0].error)}>
          一键舆情分析
        </button>
      </div>
      ...
      {showReport && (
        <div className={styles['report-modal']}>
          ...
          <div className={styles['modal-content']}>
            <h3>舆情分析报告</h3>
            <pre className={styles['report-content']}>{report}</pre>
