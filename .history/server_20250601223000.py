...
# 数据库配置
DATABASE_URL = os.getenv("DATABASE_URL", "mysql+mysqlconnector://user:password@host:3306/db_name")
...
# 定义HTTP邮件发送接口
@app.post("/send_email")
async def send_email_endpoint(request_body: EmailRequest, db: Session = Depends(get_db)):
    """通过HTTP接口发送邮件"""
    try:
        # 从请求体中获取报告内容，并生成文件名
...
# 定义获取历史报告列表的接口
@app.get("/reports")
async def get_reports(keyword: str = None, page: int = 1, page_size: int = 10, start_date: str = None, end_date: str = None, db: Session = Depends(get_db)):
    """获取所有报告列表，支持关键词搜索和分页"""
    try:
