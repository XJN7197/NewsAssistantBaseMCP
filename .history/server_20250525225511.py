...
# 定义获取历史报告列表的接口
@app.get("/reports")
async def list_reports(keyword: str = None):
    """
    列出 sentiment_reports 目录下的所有报告文件，并包含创建时间和摘要。
    支持根据关键词过滤报告文件名。
    """
    output_dir = "./sentiment_reports"
    ...
    for filename in os.listdir(output_dir):
        if filename.endswith(".md"):
            ...
            try:
                ...
                # For Unix-like systems, use os.path.getmtime (modification time) or os.path.getctime (depending on system)
                # We'll use getmtime for broader compatibility, assuming modification time is close to creation time
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    for i, line in enumerate(f):
                        if i < 5: # Read first 5 lines as summary
                            summary += line.strip() + " "
                        else:
                            break
                ...
                # Optionally, add an entry indicating an error for this file
                reports.append({
                    "filename": filename,
                    "error": f"无法读取文件信息: {e}"
                })
