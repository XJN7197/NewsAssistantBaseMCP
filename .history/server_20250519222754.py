...
# 假设 FastMCP 实例通过 .app 属性暴露 ASGI 应用程序
# 这是解决 TypeError 的可能方法
app = mcp.app
...
# 新增一个用于 HTTP 通信的工具方法
@mcp.tool()
async def http_search_news(keyword: str):
    """
    通过 HTTP 接口调用 search_google_news 工具，并返回纯净的新闻数据列表。

    参数:
        keyword (str): 关键词，如 "小米汽车"

    返回:
        list: 包含新闻标题、描述、链接的字典列表，或包含错误信息的字典
    """
    # 调用原有的 search_google_news 工具
    result_string = await search_google_news(keyword)

    # 检查是否是错误信息
    if result_string.startswith("❌"):
        return {"error": result_string}

    # 从返回的字符串中提取 JSON 数组部分
    # 假设 JSON 数组是字符串中第一个 '[' 和最后一个 ']' 之间的内容
    startIndex = result_string.find('[')
    endIndex = result_string.rfind(']')

    if startIndex != -1 and endIndex != -1 and endIndex > startIndex:
        json_array_string = result_string[startIndex : endIndex + 1]
        try:
            articles = json.loads(json_array_string)
            return articles # 返回解析后的新闻列表
        except json.JSONDecodeError:
            print(f"Failed to parse JSON from string: {json_array_string}")
            return {"error": "Failed to parse news data from backend response."}
    else:
        print(f"Could not find JSON array in response string: {result_string}")
        return {"error": "Backend response format unexpected."}
...
if __name__ == "__main__":
    ...
    # 使用 uvicorn.run() 运行 mcp.app 实例作为 ASGI 应用程序
