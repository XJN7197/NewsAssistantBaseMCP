...
@mcp.tool()
async def send_email_with_attachment(to: str, subject: str, body: str, filename: str, db: Annotated[Session, Depends(get_db)]) -> str:
    """
    发送带附件的邮件。
...
# 定义HTTP邮件发送接口
@app.post("/send_email")
async def send_email_endpoint(request: EmailRequest, db: Annotated[Session, Depends(get_db)]):
    """通过HTTP接口发送邮件"""
    try:
        # 从请求体中获取报告内容，并生成文件名
